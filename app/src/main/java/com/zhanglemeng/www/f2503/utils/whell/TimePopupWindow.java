package com.zhanglemeng.www.f2503.utils.whell;
...
import com.zhanglemeng.www.f2503.utils.whell.lib.ScreenInfo;
import com.zhanglemeng.www.f2503.utils.whell.lib.WheelTime;
...
public class TimePopupWindow extends PopupWindow implements OnClickListener {
	...
	public TimePopupWindow(Context context, Type type, final Activity activity) {
		super(context);
		this.setWidth(LayoutParams.MATCH_PARENT);
		this.setHeight(LayoutParams.WRAP_CONTENT);
//		this.setBackgroundDrawable(new BitmapDrawable());// 这样设置才能点击屏幕外dismiss窗口
//		this.setOutsideTouchable(true); //点击背景将没有反应
//		this.setAnimationStyle(R.style.timepopwindow_anim_style); //关闭动画效果
		...
		//点击其他地方消失
		final TimePopupWindow that = this;
	 ...
	 * @param date
	 */
	public void showAtLocation(Activity activity, View v,Date date) {
		backgroundAlpha(0.4f, activity);
		PopupWindowUtils.closeKeyBoard(activity);
		Calendar calendar = Calendar.getInstance();
		if (date == null)
			calendar.setTimeInMillis(System.currentTimeMillis());
		else
		...
		wheelTime.setPicker(year, month, day, hours, minute);
		update();
		super.showAtLocation(v, Gravity.BOTTOM, 0, 0);
	}
